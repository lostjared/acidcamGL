    ...
    static auto callback = [&](cv::Mat *frame, int offset, int cols, int size) {
        for(int z = offset; z <  offset+size; ++z) {
            for(int i = 0; i < cols; ++i) {
                ...
                pixel[0] = ac::wrap_cast((0.33 * pixel[0]) + (0.33 * pix[1]) + (0.33 * pix2[2]));
                pixel[1] = ac::wrap_cast((0.33 * pixel[2]) + (0.33 * pix[1]) + (0.33 * pix2[0]));
                pixel[2] = ac::wrap_cast((0.33 * pixel[1]) + (0.33 * pix[0]) + (0.33 * pix2[2]));
            }
        }
    };
