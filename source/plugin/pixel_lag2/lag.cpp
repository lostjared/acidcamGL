...
extern "C" void filter(cv::Mat  &frame) {
    ...
    int num_pixels = rand()%((frame.rows * frame.cols)/180);

    int off_x = 0 , off_y = 0, offset = 0;
    
    for(int q = 0; q < num_pixels; ++q) {
        
        int pixel_size = 16+(rand()%32);
        
        off_x = rand()%(frame.cols-pixel_size-1);
        if((q%12) == 0) {
            off_y = rand()%(frame.rows-pixel_size-1);
            offset = rand()%(collection.size()-1);
        }
