...
void Glitch_Rect_Size_RGB_drawBlock(int channel, double &alpha, int x, int y,  int w, int h, cv::Mat &frame, const cv::Mat &src) {
...
extern "C" void filter(cv::Mat  &frame) {
    ...
    for(int y = 0; y < frame.rows; y += row_size) {
        for(int x = 0; x < frame.cols; x += col_size) {
            ...
            Glitch_Rect_Size_RGB_drawBlock(channel_order, alpha, x, y, col_size, row_size, frame, collection.frames[index]);
        }
    }
